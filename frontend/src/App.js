...
class App extends Component {
  state = {
    data: [],
    cord: {id:"1", lon:"12.1", lat:"12.1", altitude:"12", name:"kaab"},
    id:"9"
  };


  componentDidMount() {
    this.setState({ data: [], cord: {id:"1", lon:"12.1", lat:"12.1", altitude:"12", name:"kaab"}, id:"9"})
    // this.getTodos() 
  ...
  handleClick(e) {
    ...
    switch (verb) {
      case 'POST':
        ...
        newObj.postData().then(() => {
          window.alert("Success, please refresh page to see your entry")})
        break;
      ...
      case 'PUT':
        ...
        newObj = new Requests(args)
        newObj.putData().then(() => {
          window.alert("Success, the row "+ this.state.id + " has been updated")})
        break;
      case 'DELETE':
        args.url = `http://127.0.0.1:8000/api/${this.state.id}`
        args.id = this.state.id
        newObj = new Requests(args)
        newObj.deleteData().then(() => {
          window.alert("Success the row "+ args.id +" has been deleted")})
        break;
      default:
        newObj = new Requests(args)
